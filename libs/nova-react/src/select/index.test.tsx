...
describe('Select', () => {
  ...
  it('should allow custom classNames', () => {
    const { container } = render(<Select className="test-class" />);
    expect(container.firstElementChild?.className).toBe('v-input test-class');
  });

  it('should permeate basic props', () => {
    const { container } = render(<Select aria-label="test-aria-label" id="test-id" role="menu" />);
  ...
  it('should allow for child components', () => {
    const { container } = render(<Select>Test Child</Select>);
