...
describe('MessageContent', () => {
  ...
  it('should allow custom classNames', () => {
    const { container } = render(<MessageContent className="test-class" />);
    expect(container.firstElementChild?.className).toBe('v-message-content test-class');
  });

  it('should allow custom tag', () => {
    const { container } = render(<MessageContent tag="button" />);
    expect(container.firstElementChild?.tagName).toBe('BUTTON');
  });

  it('should permeate basic props', () => {
    const { container } = render(<MessageContent aria-label="test-aria-label" id="test-id" role="menu" />);
  ...
  it('should allow for child components', () => {
    const { container } = render(<MessageContent>Test Child</MessageContent>);
