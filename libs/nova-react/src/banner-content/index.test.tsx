...
describe('BannerContent', () => {
  ...
  it('should allow custom classNames', () => {
    const { container } = render(<BannerContent className="test-class" />);
    expect(container.firstElementChild?.className).toBe('v-message-content test-class');
  });

  it('should allow custom tags', () => {
    const { container } = render(<BannerContent tag="button" />);
    expect(container.firstElementChild?.tagName).toBe('BUTTON');
  });

  it('should permeate basic props', () => {
    const { container } = render(<BannerContent aria-label="test-aria-label" id="test-id" role="menu" />);
  ...
  it('should allow for child components', () => {
    const { container } = render(<BannerContent>Test Child</BannerContent>);
